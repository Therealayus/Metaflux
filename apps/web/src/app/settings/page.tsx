...
export default function SettingsPage() {
  ...
  async function doSignOut() {
    try {
      ...
      setActionError(e instanceof ApiError ? e.message : "Sign out failed");
    }
  }
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        action={
          user ? (
            <button onClick={() => void doSignOut()} className="h-9 rounded-lg border border-white/10 px-4 text-sm text-zinc-200 hover:bg-white/[0.05]">
              Sign out
            </button>
          ) : undefined
        }
      ...
      {actionError ? <p className="mb-4 rounded-lg border border-red-400/25 bg-red-400/5 px-3 py-2 text-xs text-red-200">{actionError}</p> : null}
      {loading ? (
        <LoadingBlock />
      ) : !user || !org ? (
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-5">
            ...
            <p className="mt-2 text-sm text-zinc-100">{user.name ?? user.email}</p>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {orgs.map((o) => (
                <button
                  ...
                  className={`rounded-lg border px-3 py-1.5 text-sm ${o.id === org.id ? "border-indigo-400/50 bg-indigo-500/15 text-indigo-200" : "border-white/10 text-zinc-300 hover:bg-white/[0.05]"}`}
                >
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {workspaces.map((w) => (
                  <button
                    ...
                    className={`rounded-lg border px-3 py-1.5 text-sm ${w.id === workspace?.id ? "border-indigo-400/50 bg-indigo-500/15 text-indigo-200" : "border-white/10 text-zinc-300 hover:bg-white/[0.05]"}`}
                  >
                    {w.name}
                  </button>
                ))}
